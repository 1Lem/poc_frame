...
`def poc():  #自定义poc内容
    method= '<mark>get</mark>'  # {get,post,put,delete}
    ...
    poc_post_data = ''
    poc_json_data = ''
    verification = '<mark>regex</mark>'  # {'正则':'regex','响应包':'response','状态码':'status_code','json':'json'}
    re_data_keyword = '' # 响应包关键词
    regex_match=r"<mark>'id'</mark>:(.+?)<mark>;</mark>" #自定义正则匹配规则 r'r(.+?)l'
