...
def scan_urls_method(poc_url_path, poc_post_data,header,poc_files,method,verification,re_data_keyword,regex_match,poc_json_data,config_path,status_code,Secondary_verification,Secondary_verification_path):
    result = readfiles()   
        ...
        try:
            ...
            if re_data.status_code == status_code:
                ...
                with open(f'scan_out_{config_path}.txt', mode='a') as file_handle:
                    process_verification(scan_url, re_data, verification,re_data_keyword,regex_match, file_handle,Secondary_verification,Secondary_verification_path,header,url)    
            else:  
                print("不存在")  
                #print(re_data.text)  
...
def process_verification(scan_url, re_data, verification, re_data_keyword,regex_match, file_handle,Secondary_verification,Secondary_verification_path,header,url):  
    if verification == 'status_code':  
        ...
        if find_list:
            ...
            if Secondary_verification == 'true':
                scan_regex_match_url(url,find_list,Secondary_verification_path,header)  #对匹配的链接内容进行请求  
            else:
                print('异常')
...
def scan_regex_match_url(url,find_list,Secondary_verification_path,header):
    timeout_s,proxies,requests_methods = basic_setting()
